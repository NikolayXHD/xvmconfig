 ...
 */
{
  ...
  "battleLoading": {
    ...
    // Display format for the left panel (macros allowed, see macros.txt).
    // Формат отображения для левой панели (допускаются макроподстановки, см. macros.txt).
    "formatLeftVehicle": "{{vehicle}}<font face='mono' size='{{xvm-stat?13|0}}'> <font color='{{c:kb}}'>{{kb%2d~k|   }}</font> <font color='{{c:xr}}'>{{r_size=2?{{{{r==XX?XX|r%2d}}|  }}|{{r_size=5?{{r%5d|     }}|{{r%4d|    }}}}}}</font> <font color='{{c:winrate}}'>{{winrate%2d~%|   }}</font></font>",
    // Display format for the right panel (macros allowed, see macros.txt).
    // Формат отображения для правой панели (допускаются макроподстановки, см. macros.txt).
    "formatRightVehicle": "<font face='mono' size='{{xvm-stat?13|0}}'><font color='{{c:winrate}}'>{{winrate%2d~%|   }}</font> <font color='{{c:xr}}'>{{r_size=2?{{{{r==XX?XX|r%2d}}|  }}|{{r_size=5?{{r%5d|     }}|{{r%4d|    }}}}}}</font> <font color='{{c:kb}}'>{{kb%2d~k|   }}</font> </font>{{vehicle}}",
