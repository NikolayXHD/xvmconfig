 ...
 */
{
  "templates": {
    ...
    "clanIcon": {
      "enabled": true,
      "x": -15, "y": 5, "width": 16, "height": 16, "align": "center", "alpha": 90, "bindToIcon": false,
      "src": "{{clanicon}}"
    }
  },
  "statisticForm": {
    ...
    // Display format for the left panel (macros allowed, see macros.txt).
    // Формат отображения для левой панели (допускаются макроподстановки, см. macros.txt).
    "formatLeftVehicle": "{{vehicle}} <font size='{{xvm-stat?13|0}}' alpha='{{alive?#FF|#80}}'><font color='{{c:kb}}'>{{kb%2d~k}}</font> <font color='{{c:xr}}'>{{r}}</font> <font color='{{c:winrate}}'>{{winrate%2d~%}}</font></font>",
    // Display format for the right panel (macros allowed, see macros.txt).
    // Формат отображения для правой панели (допускаются макроподстановки, см. macros.txt).
    "formatRightVehicle": "<font size='{{xvm-stat?13|0}}' alpha='{{alive?#FF|#80}}'><font color='{{c:winrate}}'>{{winrate%2d~%}}</font> <font color='{{c:xr}}'>{{r}}</font> <font color='{{c:kb}}'>{{kb%2d~k}}</font></font> {{vehicle}}",
    // Display format for the left panel (macros allowed, see macros.txt).
    // Формат отображения для левой панели (допускаются макроподстановки, см. macros.txt).
